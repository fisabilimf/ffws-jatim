...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Add New Scaler</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.mas-scalers.store') }}" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('io_axis') is-invalid @enderror" 
                                ...
                                @foreach($axes as $key => $value)
                                    <option value="{{ $key }}" {{ old('io_axis') == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('technique') is-invalid @enderror" 
                                ...
                                @foreach($techniques as $key => $value)
                                    <option value="{{ $key }}" {{ old('technique') == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-8">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                Supported formats: .pkl, .joblib, .json (Max size: 10MB)
                            </small>
                            @error('scaler_file')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="form-group mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Create Scaler
                    </button>
                    <a href="{{ route('admin.mas-scalers.index') }}" class="btn btn-secondary">
