...
@section('title', 'Device Parameters Management')

@section('header')
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Device Parameters Management</h1>
...
@section('content')
    <div class="space-y-6">
        <!-- Filter Section -->
        <x-filter-bar>
            <form method="GET" action="{{ route('admin.mas-device-parameters.index') }}" class="space-y-4 md:space-y-0 md:flex md:items-center md:space-x-4">
                <!-- Search -->
                <div class="flex-1">
                    <input type="text" 
                           name="search" 
                ...
                <!-- Reset Button -->
                <a href="{{ route('admin.mas-device-parameters.index') }}" class="w-full md:w-auto px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-400 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 text-center">
                    Reset
                </a>
            </form>
        </x-filter-bar>
        ...
        <x-table>
            <x-slot name="header">
                <div class="flex justify-between items-center">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                        Device Parameters List ({{ $deviceParameters->total() }} total)
                    </h2>
                    <a href="{{ route('admin.mas-device-parameters.create') }}" 
            ...
            <x-slot name="body">
                @if($deviceParameters->count() > 0)
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($deviceParameters as $parameter)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end space-x-2">
                                                <a href="{{ route('admin.mas-device-parameters.show', $parameter) }}" 
                                                   class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">View</a>
                                                <a href="{{ route('admin.mas-device-parameters.edit', $parameter) }}" 
                                                   class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</a>
                                                <form action="{{ route('admin.mas-device-parameters.destroy', $parameter) }}" method="POST" class="inline" 
                                                      onsubmit="return confirm('Are you sure you want to delete this device parameter?')">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    <!-- Pagination -->
                    <div class="mt-4">
                        {{ $deviceParameters->links() }}
                    </div>
                @else
                    <div class="text-center py-12">
                        <div class="text-gray-500 dark:text-gray-400">
                            <i class="fas fa-cogs text-4xl mb-4"></i>
                            <p class="text-lg font-medium">No device parameters found</p>
                            <p class="mt-2">Get started by creating a new device parameter.</p>
                        </div>
                        <div class="mt-6">
                            <a href="{{ route('admin.mas-device-parameters.create') }}" 
