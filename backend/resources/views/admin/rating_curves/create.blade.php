...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Add New Rating Curve</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.rating-curves.store') }}" id="ratingCurveForm">
                @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('mas_sensor_code') is-invalid @enderror" 
                                ...
                                @foreach($sensors as $sensor)
                                    <option value="{{ $sensor->sensor_code }}" {{ old('mas_sensor_code') == $sensor->sensor_code ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('formula_type') is-invalid @enderror" 
                                ...
                                @foreach($formulaTypes as $key => $value)
                                    <option value="{{ $key }}" {{ old('formula_type') == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="form-group mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Create Rating Curve
                    </button>
                    <a href="{{ route('admin.rating-curves.index') }}" class="btn btn-secondary">
