...
const MapboxMap = ({ tickerData, onStationSelect, onMapFocus }) => {
  ...
  const activateRiverLayer = (coords = null) => {
    ...
    try {
        if (map.current.getSource('safe-areas')) {
            if (map.current.getLayer('safe-areas-fill')) {
                map.current.removeLayer('safe-areas-fill');
            }
            if (map.current.getLayer('safe-areas-border')) {
                map.current.removeLayer('safe-areas-border');
            }
            map.current.removeSource('safe-areas');
        }

        map.current.addSource('safe-areas', {
            type: 'geojson',
            data: safeAreas
        });
        ...
        if (!map.current.getLayer('water-layer')) {
            ...
            map.current.setFilter('water-layer', [
                'all',
                ['==', 'class', 'river']
            ]);
        ...
        console.error("Error activating river layer:", error);
    }
  };
  ...
  const toggleWaterEffect = () => {
    setShowWaterEffect(!showWaterEffect);
  };
    ...
    <div className="w-full h-screen overflow-hidden relative z-0">
      ...
      {selectedStationCoords && !riverLayerActive && zoomLevel > 10 && mapLoaded && (
        <button
          ...
          className="absolute bottom-16 left-4 z-10 px-4 py-2 rounded-lg shadow-lg transition-colors flex items-center bg-white hover:bg-gray-100 text-gray-800"
        >
      ...
      {selectedStationCoords && riverLayerActive && zoomLevel > 10 && (
        <>
          <button
            onClick={toggleRiverLayer}
            className="absolute bottom-28 left-4 z-10 px-4 py-2 rounded-lg shadow-lg transition-colors flex items-center bg-red-500 hover:bg-red-600 text-white"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <button
            onClick={toggleWaterEffect}
            className={`absolute bottom-16 left-4 z-10 px-4 py-2 rounded-lg shadow-lg transition-colors flex items-center ${
              showWaterEffect 
