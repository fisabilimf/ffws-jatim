...
const PredictionChart = ({ stationData, className = "" }) => {
    const chartData = useMemo(() => {
        const predictedWaterLevel = (stationData?.value || 0.4);
        const leveeData = [
            { x: 0, y: 3.5 },
            { x: 1, y: 3.5 },
            { x: 2, y: 3.5 },
        ...
        return {
            ...
            datasets: [
                ...
                // Garis horizontal penanda tinggi muka air prediksi
                {
                    label: "Tinggi Muka Air (Prediksi)",
                    data: leveeData.map(() => predictedWaterLevel),
                    borderColor: "#b91c1c", // merah gelap agar kontras
    ...
    const options = {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                mode: 'nearest',
                intersect: false,
                filter: (item) => item.dataset && item.dataset.label === 'Tinggi Muka Air (Prediksi)',
                callbacks: {
                    label: (ctx) => {
